...
description: Create a new blank Figma file. Use when the user wants to create a new Figma design or FigJam file, or when you need a new file before calling use_figma. Handles plan resolution via whoami if needed. Usage â€” /figma-create-new-file [editorType] [fileName] (e.g. /figma-create-new-file figjam My Whiteboard)
---
...
1. **User already provided a planKey** (e.g. from a previous `whoami` call or in their prompt) â†’ use it directly, skip to Step 2.

2. **No planKey available** â†’ call the `whoami` tool. The response contains a `plans` array. Each plan has a `key`, `name`, `seat`, and `tier`.
...
The tool returns:
- `file_key` â€” the key of the newly created file
- `file_url` â€” a direct URL to open the file in Figma
