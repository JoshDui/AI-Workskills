...
description: MANDATORY prerequisite â€” load this skill BEFORE every `generate_diagram` tool call. Routes to type-specific guidance (generic flowchart, architecture flowchart) and tells you when to proceed directly, when to use a different diagram type, or when the tool isn't the right fit at all.
---
...
### Unsupported â€” don't call the tool
...
Lightweight routing â€” use the first match.

| User wantsâ€¦ | Type | Next step |
|---|---|---|
...
If a flowchart is requested and it describes software infrastructure (services, datastores, queues, external integrations), route to `architecture.md` â€” not `flowchart.md`. When in doubt, ask the user.
...
4. **Reserved words** â€” don't use `end`, `subgraph`, `graph` as node IDs.
...
7. **Sequence diagrams** â€” do not use notes.
8. **Gantt charts** â€” do not use color styling.

## Step 4: Garbage in, garbage out

The quality of the generated diagram is bounded by the quality of the Mermaid you produce, which is bounded by the context you have. Before writing Mermaid, make sure you have enough real information to describe the subject accurately â€” and use whatever the current environment gives you to gather it.

Depending on what's available, useful sources of context include:

- **Source code** â€” grep/read the relevant files so the diagram reflects real service names, real edge labels, real data stores, real entry points. Walking actual routes/handlers/consumers beats recreating from memory.
- **User-provided documents** â€” a PRD, spec, meeting notes, transcript, research synthesis, onboarding doc, process write-up. Ask the user to paste or attach it if the subject isn't code.
- **Existing Figma or FigJam files** â€” if the new diagram should align with one the user already has, read it with `get_figjam` or `get_design_context` (see the `figma-use` and `figma-use-figjam` skills).
- **Other MCP servers or tools you have available** â€” issue trackers, docs sites, CRMs, analytics, internal wikis, design systems, database schemas, etc. If a connected tool holds the ground truth for what you're diagramming, pull from it rather than guessing.
- **The user themselves** â€” when the description is thin or ambiguous (unclear direction of flow, unclear scope, unclear which entities matter), ask one or two focused questions before generating. Examples: "What are the 3â€“5 main steps?", "Who owns each step?", "What triggers the next step?". One good question beats one wasted diagram.

Don't invent edges, labels, or entities to "round out" a diagram. Missing information is better than hallucinated information â€” leave a gap and flag it to the user.

## Step 5: Will the diagram need more than Mermaid can express?

Mermaid can't do everything. Sticky-note annotations tied to specific nodes, per-node domain coloring on ERDs, callouts with attached data â€” these all require composing `generate_diagram` with `use_figma` (via the [figma-use-figjam](../figma-use-figjam/SKILL.md) skill). This is the **hybrid workflow**.

It's a judgment call, not a default. Deploy it when the user's ask clearly benefits â€” skip it when the base diagram is obviously enough. Signals that say yes: user explicitly asked for notes, colors, callouts, or "X attached to each node"; they shared data that maps to specific nodes; the diagram is a shareable artifact, not a thinking sketch. Signals that say no: short/self-explanatory request, small diagram, user exploring or testing.

**If hybrid is warranted, read [references/workflow.md](./references/workflow.md) before calling `generate_diagram`** â€” it covers the pattern, two core recipes (annotations + color-coding), communication style, and failure handling. If not, proceed directly to Step 6.
...
Optional:
- `userIntent`: a short sentence describing what the user is trying to accomplish â€” helps telemetry and downstream tuning
...
Do **not** call `create_new_file` before `generate_diagram` â€” the tool creates its own file.
...
- If extensions are warranted (see Step 5), compose with `use_figma` now â€” the pattern and recipes are in [references/workflow.md](./references/workflow.md).
...
- The user is iterating on the same diagram ("try again withâ€¦", "change the labelsâ€¦").
...
2. If you want to replace the previous diagram rather than adding next to it, use the `use_figma` tool (see the `figma-use-figjam` skill) to delete the old diagram's nodes first, then call `generate_diagram` with the same `fileKey`. Or leave the old diagram and place the new one beside it â€” readers often benefit from seeing the history of attempts.

Ask the user which they prefer the first time you iterate â€” "regenerate over the old one, or keep both side-by-side?" â€” and remember their answer for subsequent iterations in the session.
